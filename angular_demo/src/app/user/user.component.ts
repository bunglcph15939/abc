...
export class UserComponent implements OnInit {
  ...
  userFillter=this.users;
// định ngihax hàm xóa khi click
 remove(userId : number){
  this.userFillter= this.userFillter.filter(function (user){
    return user.id!== userId
  })
 }
 timkiem(event:any){
  const value=event.target.value.toLowerCase()
  const xoacach=value.trim();
this.userFillter= this.users.filter(function (user){
  const lower =user.name.toLowerCase()
  return lower.indexOf(xoacach)!==-1
})
 }
 //thêm mới
 newUser={
...
onChange(event:any ,key:string){
    ...
    this.newUser={
      ...this.newUser,
      [key]:event.target.value// giá trị key trùng với tên biến của obj
    }
    console.log(this.newUser);
}
onSubmit(){
 
  if(!this.kt(this.newUser)){
    alert('Không được để trống')
    return;
  }
  //1.1 kiểm tra xem phải sửa k
  if(this.isEdit){
...
kt(obj:any){
  if(!obj.name || !obj.age|| obj.age<='0'|| !obj.phone ||obj.phone==='0' || !obj.avarta){
    return false;
  }
  return true;
}
